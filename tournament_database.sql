...
-- TEST CODE for implementation of player_standings table. NOT IN USE ANYMORE
  CREATE TABLE player_standings(
    ...
    '''TEST CODE
    Code for standings was not functioning properly so I opted to use the code for play_stands view to look at the standings.
    ...
    CREATE VIEW standings as SELECT players.id, players.name, total_wins.total_wins as wins, total_matches.total_matches as matches from players, matches where players.id = total_wins.id AND total_wins.id = total_matches.id;
    '''
    -- View for the standings for the players within the tournament which utilizes data from total wins view and total matches view to a standings record for each player.
    CREATE VIEW play_stands as SELECT players.id, players.name, (SELECT count(winner) FROM matches WHERE winner = players.id) as total_wins, (SELECT count(*) FROM matches WHERE players.id in (winner, loser)) as total_matches FROM players ORDER BY total_wins DESC, total_matches DESC;
